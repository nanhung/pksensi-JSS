...
Figure \ref{fig:SI-output} shows the concentration of rest body (\code{Ccompartment}) and the model parameters. We can find that \code{kgutabs} and \code{vdist} have higher correlation with \code{Ccompartment} in the beginning (t = 0.01 h) of post-intake duration compared with other parameters, representing that the parameters have high impact on the modeling result. The \code{ke} shows the high correlation at the later time period (t = 24.01 h). The \code{BW} does not show any obvious relationship with \code{Ccompartment}.
...
The 21 tested model parameters are defined in this step, including parameter names and probability distributions. To prevent errors, the ranges of \code{SULT_VmaxC} and \code{UGT_VmaxC} need to be adjusted from $U(0, 15)$ [@s13318-015-0253-x] to $U(0, 10)$ [@fphar201800588]. The objects \code{q} and \code{dist} are set to the type of distribution that will use to generate the parameter matrix in \pkg{GNU MCSim} (for uncertainty analysis) and \proglang{R} (for SA).
