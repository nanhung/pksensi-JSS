...
To start, we implemented the one-compartment PK model in \proglang{R}. \pkg{pksensi} allows users select the preferred method to solve the PK model, either with the \pkg{deSolve} package or with \pkg{GNU MCSim} through the compile function. This section mainly focus on how to conduct global SA with \pkg{deSolve} package with pure \proglang{R} environment. Then compare the computing time with \pkg{GNU MCSim}.

The one-compartment PK model can describe the quantity of compound in the gut lumen (\code{Agutlument}) and the rest of body (\code{Acompartmant}). The \code{Ametabolized} is the quantity of compound transform and metabolize through hepatic clearance. The \code{AUC} is the calculated area under the curve of the rest body ($mg \cdot h/L$). 
