...
The parameter values and initial states need to assign to the specific value before simulation. We used the corresponding parameter value of acetaminophen (APAP) in this example. These model parameters are derived from the *in-vivo* or *in-vitro* experiment results. The parameter value can be generated from \code{parameterize_1comp} function in \pkg{httk} package as: 
...
```{r, eval=T, fig.height=4, fig.width=4, fig.cap = '\\label{fig:ode-PK}Simulation results of one-compartment PK model.'}
plot(y)
```
...
Here we use a sample size of 200 with 10 replications. Through \code{rfast99} function, a S3 object with class 'rfast99' will be created. The \code{set.seed} can use to reproduce the same parameter matrix in the random sampling. The number of sample size determines the robustness of the result of SA. Higher number of sample size lead to narrower confidence intervals for sensitivity measurements across different replications. However, it will take a longer time in computation.
...
In addition to using \pkg{deSolve} to solve differential equations in PK model, the \pkg{GNU MCSim} can also be used, and provides better computational efficiency. To start, the users need to install \pkg{GNU MCSim} and related standard \proglang{C} compiler to solve the ODE function. To solve ODEs through \pkg{GNU MCSim}, we need to change the argument to \code{application = mcsim} in \code{compile_model}. The following \proglang{R} script will download the one-compartment PK model from https://github.com/nanhung/pksensi/blob/master/tests/pbtk1cpt.model. 
...
Based on our previous study, we proposed the heatmap visualization approach \code{heat_check} to distinguish "influential" and "non-influential" parameters with a "cut-off" point. Through the given argument \code{order}, we can select the specific order of sensitivity measurement that we're interested in (Figure \ref{fig:Heat-SI}). 
