...
To start, we implemented the one-compartment PK model in \proglang{R}. \pkg{pksensi} allows users select the preferred method to solve the PK model, either with the \pkg{deSolve} package or with \pkg{GNU MCSim} through the compile function. Running model under \pkg{GNU MCSim} native code can have a faster speed to obtain the model outputs. This section mainly focus on how to conduct global SA with \pkg{deSolve} package with pure \proglang{R} environment. Then compare the computing time with \pkg{GNU MCSim}.

The one-compartment PK model was mainly includes two state variables, which are the quantity of compound in the gut lumen (\code{Agutlument}) and the rest of body (\code{Acompartmant}). The \code{Ametabolized} is the quantity of compound transform and metabolize through hepatic clearance. The \code{AUC} is the calculated area under the curve of the rest body ($mg \cdot h/L$).
...
The parameter values and initial states need to assign to the specific condition. We used the corresponding parameter value of acetaminophen (APAP) in this example. These model parameters are derived from the *in-vivo* or *in-vitro* experiment results. The parameter value can be generated from \code{parameterize_1comp} function in \pkg{httk} package as: 
...
In addition to using \pkg{deSolve} to solve differential equations in PK model, the \pkg{GNU MCSim} can also be used, and provides better computational efficiency. To start, the users need to install \pkg{GNU MCSim} [@JSSv002i09] and related standard \proglang{C} compiler to solve the ODE function. To solve ODEs through \pkg{GNU MCSim}, we need to change the argument to \code{application = mcsim} in \code{compile_model}. The following \proglang{R} script will download the one-compartment PK model from https://github.com/nanhung/pksensi/blob/master/tests/pbtk1cpt.model. 
