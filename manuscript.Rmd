...
The parameter values and initial states can be customized to the specific condition. It can also schedule for the given dosing scenario. In the current setting, we assumed the initial condition of the intake chemical to be 1000 mg. We can use \code{initParms} and \code{initStates} functions to customize the parameter values and the initial state that will be used in the following modeling and SA. These additional functions are generated when compiling the model file. 

We used the corresponding parameter value of acetaminophen in this example. The parameter value can be generated from \code{parameterize_1comp} function in \pkg{httk} package, which includes physico-chemical and drug biological properties for over 500 chemicals. The given value of \code{vdist}, \code{ke}, and \code{kgutabs} in \pkg{httk} are 1.1 (L/kg BW), 0.23 (/h), and 2.18 (/h), respectively. The body weight is assumed to be 70 (kg).
...
We conduct global SA for the four parameters in one-compartment PK model to investigate the parameter impact on the plasma concentration during the 24-hr time period post intake. The distribution of parameter is taken to be uniform with bounds corresponding to 50% and 200% of the nominal value. Therefore, the parameter ranges are assumed to be 0.55 and 2.2 L/kg BW for \code{vdist}. The \code{ke} are ranged from 0.12 to 0.46 /h, corresponding to half-times of 1.5 and 5.8 hr. The \code{ka} are ranged 1.09 to 4.36 /h, corresponding to half-times of 0.32 and 0.64 hr. The $BW$ is assumed to a normal distribution with mean = 60 kg and sd = 5 kg. The number of samples determines the robustness of the result of SA. Higher sample numbers sizes lead to narrower confidence intervals for sensitivity measurements across different replications. However, it will take a longer time in computation. Here we use a sample size of 200 with 20 replications:
...
The Figure \ref{fig:pk-solvefun-Amet} shows that the amount of metabolized is also determined by \code{ke}. Same as \code{Ccompartment}, the \code{kgutabs} contribute about 30 - 40% variation of model output in the first hour. The \code{BW} is the least important parameter in the current analysis, and therefore can be fixed in the model calibration and applications.
...
We can find that \code{kgutabs} and \code{vdist} have higher correlation with \code{Ccompartment} in the beginning (t = 0.01 h) of post-intake duration compared with other parameters, representing that the parameters have high impact on the modeling result. The \code{ke} shows the high correlation at the lateing time (t = 24.01 h). The \code{BW} doesn't show any obvious relationship with \code{Ccompartment}.
...
In addtion to use \pkg{deSolve} to solve differential equations in PK model, the \pkg{GNU MCSim} can provide better computational efficiency. To solve ODE through \pkg{GNU MCSim}, we need to change the argument to \code{application = mcsim} in \code{compile_model}. The computing time of using \code{solve_fun} in SA is estimated as,
...
Similar to \code{solve_fun} function that can define the initial parameter and state variables' values through generated functions, the \code{solve_mcsim} also has a \code{condition} argument that can be used to give the specific input value such as oral dose, fixed parameter value or initial condition of state variable.
...
The aim of this section is to apply global SA for acetaminophen-PBPK model through \pkg{pksensi}. The model code is included in this package in https://github.com/nanhung/pksensi/blob/master/tests/pbpk_apap.model and can be generated through \code{pbpk_apap_model} function. We applied the global SA workflow to the original published model with 21 model parameters [@s13318-015-0253-x]. The descriptions of each parameter and the sampling ranges are list in Table \ref{tab:data}.
...
Generally, wide range of parameter value might cause the computating error when the solving the ODE. One of the effective ways to prevent this problem is to adjust the value of relative and absolute error tolerance to control the error appearance by resetting these parameters in a lower value. The \code{generate_infile} and \code{solve_mcsim} provide the arguments of \code{rtol} and \code{atol} that adjust the error tolerance to prevent the unwanted error. However, the modification will lower computational speed. Therefore, the alternative method to prevent the computational errors is to detect the crucial parameter range that causes the problem. Also, setting the maximum number of (internally defined) steps to higher value instead of using the default value (500) in \pkg{GNU MCSim} can prevent this problem. The maximum number of step is set to 5000 in this case. Here we seperate the global SA of APAP-PBPK model process to several steps.

### Prepare and compile the model file

The model code needs to be prepared in the following global SA workflow. After creating the \code{pbpk_apap.model} file in the working directory, the next step is to generate the executable program (\code{mcsim.pbpk_apap}) through \code{compile_model}. 
...
The 21 testing model parameters are defined in this part, including parameter name, probability distribution, and distribution parameter. To prevent the computational error, we adjusted the range of \code{SULT_VmaxC} and \code{UGT_VmaxC} from $U(0, 15)$ to $U(0, 10)$ in this case. The objects \code{q} and \code{dist} are used to set the type of distribution that will use to generate the parameter matrix in \pkg{GNU MCSim} (for uncertainty analysis) and \proglang{R} (for SA).
...
In the setting condition of simulation, The relative and absolute error tolerance (\code{rtol} & \code{atol}) were set to 1e-7 and 1e-9, respectively, to prevent the computating error. The Monte Carlo simulation is run for 1000 iteration as the assignment of \code{monte_carlo}. The input file ('sim.in') and output file ('simmc.out') will be generated under the standard ASCII format. 
...
Computational modeling has become an important technique in modern scientific research to support the hypothesis, experiment, and discoveries. Uncertainty and SA is essential knowledge for modelers to conduct the model development and examination. More computational tools had been developed for various types of modeling and simulation. The \pkg{GNU MCSim} is a powerful simulation software package, that can conduct dynamical simulation for biological-based modeling, and it's mainly built for PK research with probabilistic approaches [@bois2009gnu]. The \proglang{R} is a comprehensive statistical computing language that can conduct probabilistic programming and have tons of packages for statistical analysis and data visualization. We initiated \pkg{pksensi} that aim to conduct comprehensive and efficient global SA workflow in PK modeling, which hasn't been developed for open source communities. The current version had included \pkg{GNU MCSim} and \pkg{deSolve} as the main solver in our model assessment. Compare with \pkg{deSolve} package, we found that The \pkg{GNU MCSim} can provide better efficiency to solve the ODE in PK models. Although \pkg{pksensi} provide the features and functions to install and link with \pkg{GNU MCSim}, conducting the uncertainty and SA in PK modeling, the overall learning curve of this workflow might steep for users that are not familiar with \pkg{GNU MCSim} from model development, debugging, and testing. Therefore, the general \proglang{R} package, such as \pkg{deSolve}, can be an alternative option for \proglang{R} user. Some developed package in \proglang{R} had also provide the features to solve ODE such as \pkg{RxODE} [@wang2016tutorial] and \pkg{mrgsolve} [@R-mrgsolve]. We hope to incorporate these packages that can improve the user experience in using \pkg{pksensi} in the future.

Moreover, some fundamental packages on the CRAN repository and other software platforms had been built to perform global SA with different algorithms. The development of \pkg{pksensi} is mainly for PK models that are constructed by ODE. It provides a straightforward application to investigate the impact of parameters on model output across simulation time-points and variables. We adopted the eFSAT method, a common variance-based SA that had better efficiency and accuracy in the model assessment. It can exam the parameter effect from first order to the total order. In addition, the package can not only conduct global SA but also include the function to assess the convergence of sensitivity measurements, which is rarely discussed in most global SA studies and its software package. We also develop the function to visualize the output result that can help us distinguish the "influential" and "non-influential" parameters that can be applied in parameter fixing in the model Calibration. In addition to eFAST, Some studies applied another global SA algorithm in the physiologically and biologically based ODE models, such as compartmental absorption and transit (also known as CAT) and other system biology models [@melillo2019variance; @sumner2012methodology]. Base on our knowledge, the eFAST is a least computational expensive approch than other Monte-Carlo based Sobol' method. In future version, we expect to add additional global SA approach that had been applied in advanced studies [@glen2012estimating; @ring2017identifying]. The other features such as principle component analysis [@lamboni2011multivariate] were considered to further include in our global SA workflow.

It is worth noting that the global SA results are limited to the specific model and the given range of the model parameter. The provided conditions, such as the number of examing parameter, parameter distribution, parameter range, and the given dosing level in PK model might have a potential impact on the level of importance of each parameter. The misconduct of the setting condition might lead to the incorrect estimated result. In other words, if we have various types of experiment data (e.g. different exposure routes and biological measurements) and will use them in model calibration, it will enhance the difficulties in global SA. The complex model with a high dimension of the parameter is also a challenge in global SA, which will need higher computer demand in the model assessment. More additional applications still need to be investigated and strengthen. Overall, \pkg{pksensi} provide an additional option of performing global SA for PK model and can create robust and reproducible results for decision making in model calibration. 
