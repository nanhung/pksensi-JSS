...
In the beginning, the user need to install \pkg{GNU MCSim} [@JSSv002i09] and related standard \proglang{C} compiler to solve the ODE function. In addition, \pkg{pksensi} can link with \proglang{R} \pkg{deSolve} package, which includes the comprehensive integrator to solve ODE. The \pkg{GNU MCSim} can be installed by following the instruction in \pkg{GNU MCSim}’s manual on https://www.gnu.org/software/mcsim/mcsim.html or using the built-in function \code{mcsim_instal} in \pkg{pksensi}. The \proglang{C} compiler had originated in Linux or MacOS. The Windows user can install Rtools on https://cran.r-project.org/bin/windows/Rtools/ and set the working path of compiler (\code{/Rtools/mingw_64/bin} and \code{/Rtools/bin}) before using this function as: 
...
The aim of this section is to apply global SA for APAP-PBPK model through \pkg{pksensi}. The model describes the pharmacokinetics of parent APAP and its two metabolites glucuronide (APAP-G) and sulfate (APAP-S) through the dozens of parameters. The model code is included in this package in https://github.com/nanhung/pksensi/blob/master/tests/pbpk_apap.model and can be generated through \code{pbpk_apap_model} function. We applied the global SA workflow to the original published model with 21 model parameters [@s13318-015-0253-x]. The descriptions of each parameter and the sampling ranges are list in Table \ref{tab:data}.
...
In global SA, we have to generate the parameter matrix from the eFAST method additionally. The current setting uses sample sizes of 512 with 10 replication. 
...
This work was funded, in part, by grant 1U01FD005838 from the U.S. Food and Drug Administration and grant P42 ES027704 from the U.S. National Institute of Environmental Health Sciences. This article reflects the views of the author and should not be construed to represent FDA’s views or policies. We thank Dr. Yasha Hartberg and Dr. Barbara Gastel at the Texas A&M University and Dr. Frederic Bois from Certara for reviewing the manuscript and consultation.
