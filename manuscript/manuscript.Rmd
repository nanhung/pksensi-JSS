...
In the first example, we use a simple, one-compartment PBTK model from \pkg{httk} package \citep{JSSv079i04} to demonstrate how \pkg{pksensi} can be applied to physiologically based kinetic studies. The differential equations for this model is as:
...
The concentration of chemical in the rest of body blood can be calculate as 

$$ C_{rest} = A_{rest} / V_{dist} \cdot BW$$

In addition, $C_{rest}$ is the chemical concentration in plasma that can be used to compare with observed results in a pharmacokinetic experiment (mol/L). 

, $V_{dist}$ is the volume of distribution (L),

### Model implementations with R deSolve package

To start, we implemented one-compartment TK model in \proglang{R} by compiling the file written under the \pkg{GNU MCSim}'s model format. \pkg{pksensi} allows users select the preferred method to solve the pharmacokinetic model, either with the \pkg{deSolve} package or with \pkg{GNU MCSim} through the compile function. Running model under \pkg{GNU MCSim} native code can have faster speed to obtain the model outputs. This section demo how to conduct global sensitivity analysis with \pkg{deSolve} package. Then compare the computer time with \pkg{MCSim}.

The following \proglang{R} script will download the one-compartment PBTK model from https://github.com/nanhung/pksensi/blob/master/tests/pbtk1cpt.model.model. The model mainly includes two state variable that are the quantity of compound in gut lumen (\code{Agutlument}) and plasma (\code{Acompartmant}). The 
...
Through \code{rfast99} function, S3 object for class 'rfast99' will be created. The generated parameter will be stored as a 3-D array under the named \code{a} with the dimension of sample number, replication, and parameter number.

```{r}
dim(x$a)
```

The sample number was 800 with 4 model parameters, which generated 3,200 model evaluations. The replication was set to 20. Figure \ref{fig:rep-sampl} only plotted for 3 replications.
...
for (j in c("vdist", "ke", "kgutabs", "BW")) {
  if ( j == "BW") {
    plot(x$a[,1,j], ylab = "", cex = cex)
  } else plot(x$a[,1,j], xaxt="n", ylab = "", cex = cex)
  for (i in 2:3) {
  if ( j == "BW") {
    plot(x$a[,i,j], ylab = "", yaxt="n", cex = cex)  
    } else plot(x$a[,i,j], xaxt="n", yaxt="n", ylab = "", cex = cex)
  } 
...
To create the time-dependent sensitivity measurement, we set the time duration from 0.01 to 24.01 hours as above definition in \code{ode} function in this example. The \code{outnames}, \code{dllname}, are based on the arguments from the \code{ode} function in \pkg{deSolve} package. The details of the model structure and these arguments are defined in \code{pbtk1comp.c}. and \code{pbtk1comp_inits.R}.  The result of object x is an object of rfast99, which has specific \code{print()}, \code{plot()}, and \code{check()} method. The print function gives the sensitivity and convergence indices for main, interaction, and total order at each time point. In addition to print out the result of sensitivity analysis, the more efficient way to distinguish the influence of model parameter is to visualize them. The time-dependent sensitivity indices are shown in Figure \ref{fig:pk-solvefun}. 

```{r, eval=T, fig.cap = '\\label{fig:pk-solvefun}Time-dependent sensitivity indices of the plasma concentration estimated from one-compartment PBTK model during 24-hour time period intake. The solid line represent the total (black) and first (red) order sensitivity index with 95\\% confidence interval (polygon). The dash line is the cut-off with default value of 0.05'}
plot(out)
```
...
Here, we can see (Figure \ref{fig:pk-solvefun}) that \code{vdist} and \code{ke} are dominating the plasma concentration in the before and after 5-hour post chemical intake, respectively, representing that the elimination is a key parameter to dominate the plasma concentration. Besides, the \code{kgutabs} only plays a crucial role to determine the plasma concentration in the first hour. The relationship between concentration and the parameters can be plotted as follow: 

```{r, eval=T, fig.cap = 'The relationship between model parameter and estimated concentration at times 0.01, 2.01, and 24.01 hr'}
par(mfrow = c(3,3), mar = c(2,2,2,2), oma = c(2,2,1,1))
plot(x$a[,1,"vdist"], out$y[,1,"0.01",1], main = "vdist")
text(1, .7, "t=0.01",cex = 1.2)
plot(x$a[,1,"ke"], out$y[,1,"0.01",1], main = "ke")
plot(x$a[,1,"kgutabs"], out$y[,1,"0.01",1], main = "kgutabs")
plot(x$a[,1,"vdist"], out$y[,1,"2.01",1])
text(1, 18, "t=2.01",cex = 1.2)
plot(x$a[,1,"ke"], out$y[,1,"2.01",1])
plot(x$a[,1,"kgutabs"], out$y[,1,"2.01",1])
plot(x$a[,1,"vdist"], out$y[,1,"24.01",1])
text(1, .7, "t=24.01",cex = 1.2)
plot(x$a[,1,"ke"], out$y[,1,"24.01",1])
plot(x$a[,1,"kgutabs"], out$y[,1,"24.01",1])
...
In addition, the output are also formated as 4-D array under \code{c(model evaluation, replication, time, variable)}.
...
In addtion to apply \pkg{deSolve} to solve differential equations in PK model, the \pkg{GNU MCSim} can provide better computational efficiency in global sensitivity analysis. To solve ODE by using \pkg{GNU MCSim}, we need to change the argument to \code{application = mcsim} in \code{compile_model()}. 
...
Similar to \code{solve_fun()} function that can define the initial parameter and state values through input function, the \code{solve_mcsim()} has a `condition` argument that is used to give the specific input value such as oral dose, fixing parameter value or initial state variable.
...
Under the same given condition, it takes about 3 times computer time in (\pkg{deSolve}) than in (\pkg{GNU MCSim}) to solve this model.  
